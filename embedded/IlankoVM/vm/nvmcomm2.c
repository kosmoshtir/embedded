...
uint8_t g_nvm_runlevel = NVM_RUNLVL_BOOT;
...
uint8_t g_nvc2_address = 0x10;
uint8_t g_nvc2_msg_status = 0;
uint8_t g_nvc2_query_status = 0;
uint8_t g_nvc2_track  = 0;
uint8_t g_nvc2_le     = 0;
uint8_t g_nvc2_msgid  = 0;
uint8_t g_nvc2_lastid = 0xFF;
uint16_t g_nvc2_crc    = 0;
uint8_t g_nvc2_command;

uint8_t g_nvc2_data[NVC2_BUFFER_SIZE];
size_t g_nvc2_dsize = 0;


int8_t g_nvc2_file_open = -1;
uint16_t g_nvc2_file_pos = 0;

bool g_nvc2_query_success = false;
uint8_t g_nvc2_query_rsize = 0;
...
static inline void nvc2_send_response() {
	uint8_t msgLen = g_nvc2_query_rsize + 3;
	uint16_t crc = 0xFFFF;
...
static inline void nvc2_process_query() {
	if (nvc2_query_invalid()) {
		...
		const uint8_t command = nvc2_query_command();
		...
		if (!nvc2_query_hascmd()) {
			...
			const uint8_t respSize = nvc2_query_rsize();
			uint8_t *data = g_nvc2_data;
			...
			const uint8_t dsize = nvc2_query_dsize();
			uint8_t *data = g_nvc2_data;
...
void nvc2_check_input() {
	while (true) {
		...
		for (uint8_t i=0; !uart_available() && (i < NVC2_RECV_UART_TIMEOUT); ++i)
			{ delay(MILLISEC(1)); }

		if (uart_available()) {
			// input available
			uint8_t value = uart_read_byte();
